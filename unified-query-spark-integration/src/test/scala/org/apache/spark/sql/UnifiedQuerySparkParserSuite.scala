 ...
 */
class UnifiedQuerySparkParserSuite
  ...
  test("parsePlan should transpile PPL query to Spark SQL") {
  ...
  test("parsePlan should fall back to Spark parser for non-PPL queries") {
  ...
  test("delegate methods should pass input to Spark parser") {
    val expr = Literal(1)

...
/**
 * Flint skipping index microbenchmark that focus on skipping data structure read and write
 * performance with OpenSearch based on Spark benchmark framework.
 *
 * To run this benchmark:
 * {{{
 *   > SPARK_GENERATE_BENCHMARK_FILES=1 sbt clean "set every Test / test := {}" "integtest/test:runMain org.apache.spark.sql.benchmark.FlintSkippingIndexMicroBenchmark"
 *   Results will be written to "benchmarks/FlintSkippingIndexMicroBenchmark-<JDK>-results.txt".
 * }}}
 */
object FlintSkippingIndexMicroBenchmark extends FlintSparkBenchmark {

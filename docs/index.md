...
- `spark.datasource.flint.retry.exception_class_names`: retryable exception class name list (comma-separated). Default value is "ConnectException,ConnectionClosedException,NoHttpResponseException,ConnectionPoolTimeoutException,SocketTimeoutException", the transient connection-level faults that are safe to retry for idempotent requests. Set this to override the default list (e.g. to add or remove entries). Each entry may be a fully-qualified class name (e.g. `java.net.ConnectException`) or a simple class name (e.g. `ConnectException`); simple names match regardless of HTTP client shading/relocation.

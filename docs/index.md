...
- `spark.datasource.flint.retry.exception_class_names`: additional retryable exception class name list. by default no extra exception is added here. note: transient connection-level faults (e.g. `ConnectException`, `ConnectionClosedException`, `NoHttpResponseException`, `ConnectionPoolTimeoutException`, `SocketTimeoutException`) are always retried regardless of this setting.

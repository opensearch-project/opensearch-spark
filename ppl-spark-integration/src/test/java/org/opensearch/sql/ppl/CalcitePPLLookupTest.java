...
public class CalcitePPLLookupTest extends CalcitePPLAbstractTest {
    ...
    public void testReplace() {
        ...
        String expectedLogical = ""
            + "LogicalProject(EMPNO=[$0], ENAME=[$1], JOB=[$2], MGR=[$3], HIREDATE=[$4], SAL=[$5], COMM=[$6], DEPTNO=[$7], LOC=[$9])\n"
        ...
        String expectedResult = "" +
            "EMPNO=7782; ENAME=CLARK; JOB=MANAGER; MGR=7839; HIREDATE=1981-06-09; SAL=2450.00; COMM=null; DEPTNO=10; LOC=NEW YORK\n" +
        ...
        String expectedSparkSql = ""
            + "SELECT `EMP`.`EMPNO`, `EMP`.`ENAME`, `EMP`.`JOB`, `EMP`.`MGR`, `EMP`.`HIREDATE`, `EMP`.`SAL`, `EMP`.`COMM`, `EMP`.`DEPTNO`, `t`.`LOC`\n"
            + "FROM `scott`.`EMP`\n"
    ...
    public void testReplaceAs() {
        ...
        RelNode root = getRelNode(ppl);
        String expectedLogical = ""
            + "LogicalProject(EMPNO=[$0], ENAME=[$1], MGR=[$3], HIREDATE=[$4], SAL=[$5], COMM=[$6], DEPTNO=[$7], JOB=[COALESCE($9, $2)])\n"
            + "  LogicalJoin(condition=[=($7, $8)], joinType=[left])\n"
            + "    LogicalTableScan(table=[[scott, EMP]])\n"
            + "    LogicalProject(DEPTNO=[$0], LOC=[$2])\n"
            + "      LogicalTableScan(table=[[scott, DEPT]])\n";
        ...
        String expectedResult = "" +
            "EMPNO=7782; ENAME=CLARK; MGR=7839; HIREDATE=1981-06-09; SAL=2450.00; COMM=null; DEPTNO=10; JOB=NEW YORK\n" +
        ...
        String expectedSparkSql = ""
            + "SELECT `EMP`.`EMPNO`, `EMP`.`ENAME`, `EMP`.`MGR`, `EMP`.`HIREDATE`, `EMP`.`SAL`, `EMP`.`COMM`, `EMP`.`DEPTNO`, COALESCE(`t`.`LOC`, `EMP`.`JOB`) `JOB`\n"
            + "FROM `scott`.`EMP`\n"
            + "LEFT JOIN (SELECT `DEPTNO`, `LOC`\n"
            + "FROM `scott`.`DEPT`) `t` ON `EMP`.`DEPTNO` = `t`.`DEPTNO`";
        verifyPPLToSparkSQL(root, expectedSparkSql);
    ...
    public void testMultipleLookupKeysReplace() {
        String ppl = "source=EMP | eval newNO = DEPTNO | lookup DEPT DEPTNO as newNO, DEPTNO replace LOC as JOB";
        RelNode root = getRelNode(ppl);
        String expectedLogical = ""
    ...
    public void testAppendAs() {
        ...
        RelNode root = getRelNode(ppl);
        String expectedLogical = ""
            + "LogicalProject(EMPNO=[$0], ENAME=[$1], MGR=[$3], HIREDATE=[$4], SAL=[$5], COMM=[$6], DEPTNO=[$7], JOB=[COALESCE(COALESCE($2, $9), $2)])\n"
            + "  LogicalJoin(condition=[=($7, $8)], joinType=[left])\n"
            + "    LogicalTableScan(table=[[scott, EMP]])\n"
            + "    LogicalProject(DEPTNO=[$0], LOC=[$2])\n"
            + "      LogicalTableScan(table=[[scott, DEPT]])\n";
        ...
        String expectedResult = "" +
            "EMPNO=7782; ENAME=CLARK; MGR=7839; HIREDATE=1981-06-09; SAL=2450.00; COMM=null; DEPTNO=10; JOB=MANAGER\n" +
        ...
        String expectedSparkSql = ""
            + "SELECT `EMP`.`EMPNO`, `EMP`.`ENAME`, `EMP`.`MGR`, `EMP`.`HIREDATE`, `EMP`.`SAL`, `EMP`.`COMM`, `EMP`.`DEPTNO`, COALESCE(COALESCE(`EMP`.`JOB`, `t`.`LOC`), `EMP`.`JOB`) `JOB`\n"
            + "FROM `scott`.`EMP`\n"
            + "LEFT JOIN (SELECT `DEPTNO`, `LOC`\n"
            + "FROM `scott`.`DEPT`) `t` ON `EMP`.`DEPTNO` = `t`.`DEPTNO`";
        verifyPPLToSparkSQL(root, expectedSparkSql);
    ...
    public void testMultipleLookupKeysAppend() {
        String ppl = "source=EMP | eval newNO = DEPTNO | lookup DEPT DEPTNO as newNO, DEPTNO append LOC as COMM";
        RelNode root = getRelNode(ppl);
        String expectedLogical = ""
            + "LogicalJoin(condition=[=($7, $8)], joinType=[inner])\n"
            + "  LogicalTableScan(table=[[scott, EMP]])\n"
    ...
    public void testLookupAll() {
        ...
        String expectedResult = "" +
            "EMPNO=7782; ENAME=CLARK; JOB=MANAGER; MGR=7839; HIREDATE=1981-06-09; SAL=2450.00; COMM=null; DEPTNO=10; DEPTNO0=10; DNAME=ACCOUNTING; LOC=NEW YORK\n" +

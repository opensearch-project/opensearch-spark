...
class PPLLogicalPlanInputTranslatorTestSuite
  ...
  test("test csv file source relation") {
    ...
    val scanPlan = planTransformer.visit(
      plan(pplParser, "input 'ppl-spark-integration/src/test/resources/people.csv'", false),
      context)
  ...
  test("test gz compressed csv file source relation") {
    ...
    val scanPlan = planTransformer.visit(
      plan(
        pplParser,
        "input 'ppl-spark-integration/src/test/resources/people.csv.gz'",
        false),
      context)
  ...
  test("test unsupported compression") {
    ...
    val thrown = intercept[UnsupportedOperationException] {
      planTransformer.visit(
        plan(
          pplParser,
          "input 'ppl-spark-integration/src/test/resources/head.csv.brotli'",
          false),
        context)
    }
  ...
  test("test unsupported file format") {
    ...
    val thrown = intercept[UnsupportedOperationException] {
      planTransformer.visit(
        plan(pplParser, "input 'ppl-spark-integration/src/test/resources/head.txt'", false),
        context)
    }

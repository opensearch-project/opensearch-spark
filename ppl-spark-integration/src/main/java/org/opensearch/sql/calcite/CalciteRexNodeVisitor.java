...
//                return rexBuilder.makeIntervalLiteral(BigDecimal.valueOf((long) node.getValue()));
    ...
    public RexNode visitAnd(And node, CalcitePlanContext context) {
        final RelDataType booleanType = context.getRexBuilder().getTypeFactory().createSqlType(SqlTypeName.BOOLEAN);
        final RexNode left = analyze(node.getLeft(), context);
        final RexNode right = analyze(node.getRight(), context);
        return context.getRexBuilder().makeCall(booleanType, org.apache.calcite.sql.fun.SqlStdOperatorTable.AND, List.of(left, right));
    }
    ...
    public RexNode visitOr(Or node, CalcitePlanContext context) {
        final RexNode left = analyze(node.getLeft(), context);
        final RexNode right = analyze(node.getRight(), context);
        return context.getRelBuilder().or(left, right);
    }
    ...
    public RexNode visitXor(Xor node, CalcitePlanContext context) {
        ...
        final RexNode left = analyze(node.getLeft(), context);
        final RexNode right = analyze(node.getRight(), context);
        return context.getRexBuilder().makeCall(booleanType, SqlStdOperatorTable.BIT_XOR, List.of(left, right));
    }

    @Override
    public RexNode visitNot(Not node, CalcitePlanContext context) {
        final RexNode expr = analyze(node.getExpression(), context);
        return context.getRelBuilder().not(expr);
    }
    ...
    public RexNode visitCompare(Compare node, CalcitePlanContext context) {
        ...
        final RexNode left = analyze(node.getLeft(), context);
        final RexNode right = analyze(node.getRight(), context);
        return context.getRexBuilder().makeCall(booleanType, translate(node.getOperator()), List.of(left, right));
    }
    ...
    public RexNode visitQualifiedName(QualifiedName node, CalcitePlanContext context) {
        if (context.isResolvingJoinCondition()) {
            ...
            if (parts.size() == 1) { // Handle the case of `id = cid`
                try {
                    return context.getRelBuilder().field(2, 0, parts.get(0));
                } catch (IllegalArgumentException i) {
                    return context.getRelBuilder().field(2, 1, parts.get(0));
                }
            } else if (parts.size() == 2) { // Handle the case of `t1.id = t2.id` or `alias1.id = alias2.id`
                return context.getRelBuilder().field(2, parts.get(0), parts.get(1));
            } else if (parts.size() == 3) {
                throw new UnsupportedOperationException("Unsupported qualified name: " + node);
            }
        }
        String qualifiedName = node.toString();
        List<String> currentFields = context.getRelBuilder().peek().getRowType().getFieldNames();
        if (currentFields.contains(qualifiedName)) {
            return context.getRelBuilder().field(qualifiedName);
        ...
        if (fieldMap.containsKey(qualifiedName)) {
            return context.getRelBuilder().field(fieldMap.get(qualifiedName));
        } else {
            return null;
        }
    }
    ...
    public RexNode visitAlias(Alias node, CalcitePlanContext context) {
        ...
        return context.getRelBuilder().alias(expr, node.getName());
    }
    ...
    public RexNode visitSpan(Span node, CalcitePlanContext context) {
        ...
        RexBuilder rexBuilder = context.getRexBuilder();
        RelDataTypeFactory typeFactory = context.getRexBuilder().getTypeFactory();
        SpanUnit unit = node.getUnit();
        if (isTimeBased(unit)) {
            String datetimeUnitString = DataTypeTransformer.translate(unit);
            RexNode interval = context.getRexBuilder().makeIntervalLiteral(
            ...
            return rexBuilder.makeCall(
                ...
                List.of(
                    rexBuilder.makeCall(typeFactory.createSqlType(SqlTypeName.DOUBLE),
...
//    }
    ...
    public RexNode visitLet(Let node, CalcitePlanContext context) {
        ...
        return context.getRelBuilder().alias(expr, node.getVar().getField().toString());
    }
    ...
    public RexNode visitFunction(Function node, CalcitePlanContext context) {
        ...
        return context.getRexBuilder().makeCall(translate(node.getFuncName()), arguments);
    }
}
